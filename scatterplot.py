...
if __name__ == '__main__':
    ...
    nodegrp = ["50", "100", "500"]
    nodemrk = ["x", "d", "o"]
    edgecolors=["face", "face", "black"]
    dcolors = ["red", "orange", "lime", "b", "cyan"]

    root = ""
    rstfile = root + "RMSETCdata-TMP5LC7.csv"
    plotfile = root + "node-all.png"
    ...
    ax.set_xlabel("TC Err (from 112.1 W/(m·K) :300 K)", fontsize=12)
    ax.set_ylabel(r'force/RMSE (meV/$\AA$)', fontsize=12)
    ax.grid(True)
    ...
    for i in range(len(nodegrp)):
        ...
        for j, dtn in enumerate(RMSETCd[i]):
            ax.scatter(dtn[1], dtn[0], c=dtn[2], marker=nodemrk[i], edgecolors=edgecolors[i],s=25, alpha=0.5)
    ...
    for i in range(len(nodegrp)):
        lbl = "nodes=" + nodegrp[i]
        ax1.scatter(dtn[1], dtn[0], s=20, edgecolors='black', color='black', alpha=0.3, marker=nodemrk[i],label=lbl)
